...
def get_movie_links(page, url):
	...
	if 'allmovie' in url:
		return ['https://www.allmovie.com/movie/'+re.split('\?|\/',link)[2] for link in links if link.startswith('/movie')]
	elif 'imdb' in url:
		return ['https://www.imdb.com/title/'+re.split('\?|\/',link)[2] for link in links if link.startswith('/title')]
