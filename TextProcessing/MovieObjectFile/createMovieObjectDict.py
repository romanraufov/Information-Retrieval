...
csv2convert = "C:\\Users\\chris\\OneDrive\\Documenten\\IR_DS2019\\TextProcessing\\MovieObjectFile\\combineddata_rawsummaries.csv"
...
with open(csv2convert, "r", encoding="utf8") as csvMovie:
    ...
    for row in reader:
        ...
        if counter > 0:
            ...
            movieObjectDict[row[1]] = {'title': row[0], 'cleantitle': row[1], 'year': row[2], 'imdb_summary':row[3], "positive_sentiment":row[23], "negative_sentiment":row[24], "imdb_url":row[10], "tmdb_url":row[11], "rottentomatoes_url":row[12], "allmovie_url":row[13], "rottentomatoes_tomatometer":row[17], "rottentomatoes_audiencescore":row[18], "allmovie_rating":row[21], "rottentomatoes_audiencerating":row[19], "imdb_rating":row[15], "allmovie_summary":row[7], "rottentomatoes_summary":row[6], "flixable_summary":row[5], "tmdb_summary":row[4], "wiki_summary":row[8]}
    csvMovie.close()
